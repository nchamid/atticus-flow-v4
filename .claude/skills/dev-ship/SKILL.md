...
no GitHub round-trip. If the change breaks `dev`, the user runs `/undo`
to revert.
...
`/undo` can revert it cleanly with `git revert -m 1 HEAD` even when the
...
> integration branch) and pushed. Run `/undo` if anything broke."*
