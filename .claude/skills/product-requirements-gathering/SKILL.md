...
## Step 0.5: Create the project folder — AFTER registry check, BEFORE interview

Once you've confirmed this is a new solution (not a registry duplicate), set up a dedicated project folder so the intake interview produces a clean, self-contained project structure that downstream Design, Development, and QA teammates can find later. Analysts run Claude Code from a parent folder; this skill creates a new subfolder per project inside it.

**Use your tools** (bash `mkdir`, Write) to create the folder structure. Don't just describe what you'd do — actually create it.

### Generate a project slug

From the request the Analyst described, propose a short kebab-case slug for the project (lowercase, hyphens, no spaces or special characters). Examples:
...
Confirm with the Analyst: _"I'll set up the project folder as `./<slug>/` in your current working directory. Sound right, or do you want to rename it?"_ Adjust if they push back. Keep slugs short and clear — they will be referenced in the Solution Registry later.

### Actions to take

**For the rest of this skill, work inside the new project folder.** Every edit to solution-requirements.md during the interview happens in `templates/artifacts/docs/product/solution-requirements.md`.
...
1. **Confirm you're writing to the project folder created in Step 0.5** — the file path should be `./<slug>/solution-requirements.md`, not the master template at the firm root. If you're not sure which folder you're in, run `pwd` and adjust before saving.
...
7. **Save the file** to the project folder created in Step 0.5 as `<slug>/solution-requirements.md`. The folder already exists; just overwrite the templated copy with the completed version.
