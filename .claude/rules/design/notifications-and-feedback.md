---
name: MWS Notifications & Feedback
...
version: 1.0.0
---
# MWS Notifications & Feedback
How the system talks back to the user. Different from `MWS Disclosure Surfaces` (which is about revealing user-initiated content) — this is about the system communicating state.
## Decision matrix
| Surface | Use when |
|---|---|
...
| **Modal** | (See `MWS Disclosure Surfaces`) — only when blocking is required |
## Toasts
...
**Reminder:** Per the MWS theme-stable foreground rule, text on alert fills (`--color-error`, `--color-success`, `--color-warning`, and the pale fills) is **always navy**, never the themed `--text-primary`.
...
- Animation timing matches MWS motion tokens; never instant.
