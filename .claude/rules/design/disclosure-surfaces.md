...
version: 1.0.0
---
# MWS Disclosure Surfaces
Decisions about *how to reveal* additional content or actions. Pairs with `MWS Navigation & IA` for "where things live."
## Decision matrix
...
- **Modal action row must `flex-wrap: wrap`.** Two MWS buttons (140px min-width each + gap) won't fit a typical 256px modal-content area at narrow widths. Without `flex-wrap`, the second button gets cut off the right edge of the modal.
...
- Use `border-radius` > 2px on disclosure surfaces (violates MWS radius rule)
