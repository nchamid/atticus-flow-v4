...
## Inputs in flex rows need `min-width: 0`
A native `<input>` has an intrinsic width of ~200px (from the default `size` attribute). Inside a flex container alongside other items (icon button + input + send button, label + input + clear button), the input refuses to shrink below 200px, pushing the row past the viewport on narrow phones.

```css
/* For any <input> with flex: 1 inside a flex row */
.chat-input input,
.input-row input { flex: 1; min-width: 0; }
```

Same fix applies to `<select>` and `<textarea>` for the same reason. See `MWS Responsive & Mobile` ("Native `<input>` intrinsic width").
...
- Put an `<input>` in a flex row with `flex: 1` but no `min-width: 0` — the input's ~200px intrinsic width pushes the row off-screen on narrow viewports.
