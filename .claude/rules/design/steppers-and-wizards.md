...
*Pattern B — Single-step focus:* show only the current step prominently ("Step 2 of 4 · Processing Scenarios") plus a thin progress bar. Tap to expand into the compact horizontal stepper.
...
- Primary action is **Continue** — verb + noun. Final step names the outcome ("Submit estimate", "Create project").
...
**Info stepper:** *calculation flows* where each step produces a value worth showing — estimators, financial models, configuration wizards. Each step is a wider card with: step number (eyebrow), step name (Mix 22pt), current value/status below (`412037 · 0042`, `$2,889,796`). Active step gets a 2px top accent bar instead of a filled circle.
