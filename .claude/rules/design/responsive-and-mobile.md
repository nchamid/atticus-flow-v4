...
## Reduced motion
Respect `prefers-reduced-motion: reduce` on every animation — replace movement with opacity-only transitions. Stricter on mobile (motion sickness in transit).
