...
---
# MWS Accessibility
WCAG 2.2 Level AA is the **minimum bar** for every MWS surface. Not negotiable.
...
- MWS button (140×36) — pad the click target if visual size is smaller
...
## MWS-specific
The MWS palette has known-good pairs (navy on white = 19:1, navy on teal = 7.4:1). Confirm any other pairing. The pale-bg + navy-text rule (theme-stable foreground) exists partly for contrast — never override. Focus rings use `--focus-ring` which flips per theme — test both.
