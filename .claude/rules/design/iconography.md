...
- **On pale backgrounds** (theme-stable surfaces): always navy, never `--icon-default`. See the MWS theme-stable foreground rule.
- **Interactive icons** inherit `var(--accent-interactive)` on hover (blue light / teal dark).
- **Disabled icons:** `opacity: 0.4`.

Never use color outside the MWS palette.
...
## MWS-specific
- The MWS rule "no `<i>` tags for icons" applies to **production React code** — use Phosphor React components there.
...
- Use color outside the MWS palette for icons
