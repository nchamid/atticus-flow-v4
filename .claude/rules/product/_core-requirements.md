...
1. **Intake in Claude Code (this folder).** Run the product-requirements-gathering skill end-to-end. The skill automatically creates a dedicated project folder (`./<slug>/`) with a templated `artifacts/docs/product/solution-requirements.md`, a `README.md`, and `sample-data/` and `prototypes/` subfolders before the intake interview begins — the Analyst does not manually set up folder structure. The skill then produces a populated `artifacts/docs/product/solution-requirements.md` with confirmed sensitivity classification, tier, and acceptance criteria. Do not skip ahead to prototyping until the brief is complete.
